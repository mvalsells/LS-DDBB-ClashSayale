...
SELECT j.tag_jugador AS tag_j, j.nom AS nom_j, SUM(c.or_) AS or_guanyat, SUM(c.experiencia) AS expereincia_guanyada
FROM completen AS c
JOIN jugador AS j on c.tag_jugador = j.tag_jugador
GROUP BY j.tag_jugador
ORDER BY or_guanyat DESC;

SELECT m.id_missio, m.titol AS titol_missio, COUNT(DISTINCT c.id_arena) AS num_arenes
FROM missio AS m
